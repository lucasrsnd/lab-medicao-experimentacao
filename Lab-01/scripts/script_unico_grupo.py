"""
Script UNICO do grupo - consulta GraphQL para Lab01S01.

Issues:
    - Integrar extracoes individuais no script unico de consulta GraphQL [S01] (#7)
    - Implementar requisicao automatica [S01] (#8)

Junta num unico request GraphQL os campos ja validados individualmente por cada
integrante (ver os outros runners em scripts/), evitando 1 requisicao por RQ, e
executa a coleta dos 100 repositorios pedidos pela S01 de ponta a ponta, sem
intervencao manual: pagina automaticamente (blocos de PAGE_SIZE) e persiste o
resultado bruto em CSV e JSON.

Cobertura das RQs (marcar [x] conforme cada RQ for integrada por quem e responsavel):
    [x] RQ01 - createdAt                  (gustavoprehl)
    [x] RQ02 - pull requests aceitas       (gustavoprehl)
    [x] RQ03 - releases.totalCount         (lucasrsnd)
    [x] RQ04 - pushedAt                    (lucasrsnd)
    [x] RQ05 - primaryLanguage             (DaviSantos23)
    [x] RQ06 - issues abertas/fechadas     (DaviSantos23)
    [ ] RQ07 - derivada (sem campo proprio) - ver scripts/rq07_analise.py

Paginacao: pedir os 100 repositorios numa unica consulta (first: 100) da 502 na
API do GitHub por causa dos campos aninhados (releases, pullRequests, issues) -
funciona ate first: 50. `src.github_client.paginate()` busca em blocos de
PAGE_SIZE e junta o resultado. Paginacao para 1000 repositorios (Lab01S02) reusa
a mesma mecanica, so mudando TOTAL_REPOS/PAGE_SIZE.

Uso (a partir de Lab-01/):
    1) copie .env.example para .env e preencha GITHUB_TOKEN
    2) pip install -r requirements.txt
    3) pip install -e .          (instala config/src em modo editavel)
    4) python scripts/script_unico_grupo.py
"""

from __future__ import annotations

from pathlib import Path

from config import load_github_token

from src.export import salvar_csv, salvar_json
from src.github_client import paginate
from src.metrics import (
    extract_rq01_idade_anos,
    extract_rq02_prs_aceitas,
    extract_rq03_total_releases,
    extract_rq04_dias_desde_atualizacao,
    extract_rq05_linguagem,
    extract_rq06_razao_issues,
)
from src.queries import QUERY_UNICO_S01

TOTAL_REPOS = 100  # Lab01S01 pede consulta para 100 repositorios
PAGE_SIZE = 10  # a API de search do GitHub apresenta 502 intermitente (documentado,
# fora do nosso controle - ver comentario em src/queries) mesmo abaixo do limite de
# complexidade; paginas menores parecem reduzir a chance de bater nisso
DATA_DIR = Path(__file__).resolve().parent.parent / "data" / "raw"

CAMPOS_CSV = [
    "nameWithOwner",
    "stargazerCount",
    "createdAt",
    "idade_anos",
    "prs_aceitas",
    "total_releases",
    "pushedAt",
    "dias_desde_atualizacao",
    "linguagem_primaria",
    "total_issues",
    "issues_fechadas",
    "razao_fechadas",
]


def buscar_repositorios(token: str) -> list[dict]:
    nodes = paginate(QUERY_UNICO_S01, {}, token, total=TOTAL_REPOS, page_size=PAGE_SIZE, timeout=60)

    repos = []
    for node in nodes:
        node["idade_anos"] = extract_rq01_idade_anos(node)

        node["prs_aceitas"] = extract_rq02_prs_aceitas(node)
        node.pop("pullRequests", None)

        node["total_releases"] = extract_rq03_total_releases(node)
        node.pop("releases", None)

        node["dias_desde_atualizacao"] = extract_rq04_dias_desde_atualizacao(node)

        node["linguagem_primaria"] = extract_rq05_linguagem(node)
        node.pop("primaryLanguage", None)

        total, closed, razao = extract_rq06_razao_issues(node)
        node["total_issues"] = total
        node["issues_fechadas"] = closed
        node["razao_fechadas"] = razao
        node.pop("issues_total", None)
        node.pop("issues_closed", None)

        repos.append(node)
    return repos


def main() -> None:
    token = load_github_token()
    repos = buscar_repositorios(token)

    print(f"Consulta unica S01 - {len(repos)} repositorio(s) coletados (paginado, sem intervencao manual).")

    caminho_json = salvar_json(repos, DATA_DIR / "repositorios_s01.json")
    print(f"Resultado bruto salvo em: {caminho_json}")

    caminho_csv = salvar_csv(repos, CAMPOS_CSV, DATA_DIR / "repositorios_s01.csv")
    print(f"Resultado salvo em: {caminho_csv}")


if __name__ == "__main__":
    main()
