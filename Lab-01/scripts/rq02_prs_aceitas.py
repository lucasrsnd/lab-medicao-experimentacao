...
Uso (a partir de Lab-01/):
    ...
    4) python scripts/rq02_prs_aceitas.py
"""
...
from src.metrics import extract_rq02_prs_aceitas
from src.queries import QUERY_RQ02_PRS_ACEITAS
...
def buscar_amostra(token: str) -> list[dict]:
    data = run_query(QUERY_RQ02_PRS_ACEITAS, {"sampleSize": SAMPLE_SIZE}, token)
    repos = data["search"]["nodes"]
    for repo in repos:
        repo["prs_aceitas"] = extract_rq02_prs_aceitas(repo)
...
def validar(repos: list[dict]) -> list[str]:
    return (
        ...
        + validar_campo_presente(repos, "prs_aceitas")
        + validar_campo_nao_negativo(repos, "prs_aceitas")
    )
...
def main() -> None:
    ...
    caminho_csv = salvar_csv(
        repos,
        ["nameWithOwner", "stargazerCount", "prs_aceitas"],
        DATA_DIR / "rq02_sample.csv",
    )
