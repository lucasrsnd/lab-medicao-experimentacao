...
def calcular_dias_desde_atualizacao(pushed_at: str) -> int:
    atualizado_em = datetime.fromisoformat(pushed_at.replace("Z", "+00:00"))
    agora = datetime.now(timezone.utc)
    return (agora - atualizado_em).days
...
def validar(repos: list[dict]) -> list[str]:
    ...
    for repo in repos:
        ...
        if not repo.get("pushedAt"):
            problemas.append(f"{nome}: pushedAt ausente/nulo")
            continue
        dias = calcular_dias_desde_atualizacao(repo["pushedAt"])
        if dias < 0:
            problemas.append(f"{nome}: dias desde atualizacao negativo ({dias}) - checar timezone/parse")
        repo["dias_desde_atualizacao"] = dias
    return problemas
...
def salvar_csv(repos: list[dict]) -> Path:
    ...
    caminho = saida_dir / "rq04_sample.csv"
    with caminho.open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(
            f, fieldnames=["nameWithOwner", "stargazerCount", "pushedAt", "dias_desde_atualizacao"]
        )
        writer.writeheader()
